package main.java.com.sample.app;
	...
	private void actionInitialize(){
		...
		btnSignIn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				//int lenght = (int)(math.log10(username)+1);
				//string un = username.lenght();
				//int length = String.valueOf(username).length();
				String password = String.valueOf(textPassword.getPassword());
				System.out.println("username:"+username);
				System.out.println("password:"+password);
				
				//kondisi dimana username dan password kosong
				while (username.isEmpty() && password.isEmpty()) {
					JOptionPane.showMessageDialog(null, "Mohon masukan username dan password anda", "Error", JOptionPane.ERROR_MESSAGE);
					initialize(frmLoginPage);
				}

				//kondisi dimana username yang dimasukkan bukan angka
				if (isNumber(username) == false) {
					JOptionPane.showMessageDialog(null, "Harap masukan angka", "Warning!", JOptionPane.ERROR_MESSAGE);
					initialize(frmLoginPage);
				//kondisi dimana input username melebihi space yang disediakan
				}/*else if (username.lenght() > 10) {
					JOptionPane.showMessageDialog(null, "Maximum inputan 10 digit", "Warning!", JOptionPane.ERROR_MESSAGE);
					initialize(frmLoginPage);
				}*/
				
				//kondisi dimana username dan password sesuai dengan npm dan tanggal lahir yang telah di set
				if (username.equalsIgnoreCase("202310067") && password.equalsIgnoreCase("171202")) {
					JOptionPane.showMessageDialog (null,"Selamat datang di aplikasi Perpustakaan IBIK", "Halo!", JOptionPane.INFORMATION_MESSAGE);
				//kondisi dimana username dan password tidak sesuai dengan npm dan tanggal lahir yang telah di set
				}else if (username != "202310067" && password != "171202") {
